...
__device__ void swap(int id1, int id2, int32_t* array){
    int tmp;
...
__global__ void parInit_kernel(const int N, const int PNN, 
    ...
    if (j < N){
        ...
        if (j==0){ // 'j' is cell id
            for (int i=0; i<Ms0; i++){ // 'i' is particle id
                cellid[i] = j;
            ...
            for (int i=Tol_cumj1; i<Tol_cumj2; i++){
                ...
                // v2
                particlesIncell[j * np_max[0] + (i - Tol_cumj1)] = i;
            }
        ...
        // shuffle particles in each cell
        for (int i=0; i <np_max; i++){
            cid = round(curand_uniform (&states) * double(np_max));
            cid = j * np_max + cid;
            swap(j, cid, particlesIncell);
        }
    }
}
    ...
    AT_DISPATCH_FLOATING_TYPES(
        x.type(), "polInit_cuda", ([&] {
          parInit_kernel<scalar_t><<<block_0, thread_0, 0, stream>>>(
              ...
              layer.data<int32_t>(), particlesIncell.data<int32_t>(),
              np_max.data<int32_t>());
        }));
