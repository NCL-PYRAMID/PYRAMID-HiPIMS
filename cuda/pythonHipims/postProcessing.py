...
def multi_exportRaster_tiff(DEM_path, outPutPath, GPU_num):
    ...
    result_list = glob.glob(outPutPath + '/*cuda:0*.pt')
...
if __name__ == "__main__":
    if case == 0:
        ...
        from glob import glob
        cellSize = 10.
        result_list = glob('/home/cvjz3/overland/jh_1st_moreGauges_10_2nd/output/' +
                           '/h_tensor*.pt')
        result_list += glob('/home/cvjz3/overland/jh_1st_moreGauges_10_2nd/output/' +'/wl_tensor*.pt')
        result_list += glob('/home/cvjz3/overland/jh_1st_moreGauges_10_2nd/output/' + '/qx_tensor*.pt')
        result_list += glob('/home/cvjz3/overland/jh_1st_moreGauges_10_2nd/output/' + '/qy_tensor*.pt')
        device = torch.device("cuda", 1)

        y = range(int(1200. / cellSize))
        x = range(int(1620. / cellSize))
        for i in range(len(result_list)):
            internal_data = torch.load(result_list[i])
            internal_data = internal_data.resize(len(y), len(x))  # print(dem)
            # sec_data = (internal_data[int(len(x)/2)]+internal_data[int(len(x)/2)-1])/2.
            fig, ax = plt.subplots()
            sec_data = internal_data[:, 80]
            sec_data_2 = internal_data[:, 81]
            
            # sec_data = internal_data[50, :]
            # sec_data_2 = internal_data[51, :]
            # sec_data_2 = internal_data[:, 81*2+1]
            # sec_data = internal_data[:, 80*2]
            sec_data = sec_data.cpu().numpy()            
            sec_data_2 = sec_data_2.cpu().numpy()
            plt.plot(y, sec_data)
            plt.plot(y, sec_data_2)
            
            # sec_data = internal_data[:, 79*2+1]
            # sec_data_2 = internal_data[:, 82*2]
            # sec_data = internal_data[52, :]
            # sec_data_2 = internal_data[53, :]
            
            sec_data = internal_data[:, 79]
            sec_data_2 = internal_data[:, 82]
            sec_data = sec_data.cpu().numpy()            
            sec_data_2 = sec_data_2.cpu().numpy()
            plt.plot(y, sec_data)
            plt.plot(y, sec_data_2)
        ...
        multi_exportRaster_tiff(
            ...
            '/home/cvjz3/Luanhe_case/output', 2)
        # multi_exportRaster_tiff(
