...
hydroStaticReconstruction(scalar_t *field_cell_l, scalar_t *field_cell_r,
  ...
  // =========================================================
  // hydrostatic reconstruction from Jiaheng Zhao_2
  // =========================================================
  ...
  if ((field_cell_l[4] - field_cell_r[4]) * (field_l[4] - field_r[4]) < 0.0) {
    auto temp = (field_l[4] + field_r[4]) / 2.0;
    field_l[4] = temp;
    field_r[4] = temp;
    field_l[0] = field_l[1] - field_l[4];
    field_r[0] = field_r[1] - field_r[4];
  }
...
hllcFluxSolver(scalar_t *field_c, scalar_t *field_cell_r, scalar_t *field_l,
  ...
  auto ux_r = field_r[0] < h_small ? 0.0 : field_r[2] / field_r[0];
  auto uy_r = field_r[0] < h_small ? 0.0 : field_r[3] / field_r[0];
  // auto ux_l = field_c[0] < h_small ? 0.0 : field_c[2] / field_c[0];
  // auto uy_l = field_c[0] < h_small ? 0.0 : field_c[3] / field_c[0];

  // auto ux_r = field_cell_r[0] < h_small ? 0.0 : field_cell_r[2] /
  // field_cell_r[0];
  // auto uy_r = field_cell_r[0] < h_small ? 0.0 : field_cell_r[3] /
  // field_cell_r[0];
  ...
  //   ux_l = field_c[0] < h_small ? 0.0 : field_c[2] / field_c[0];
  //   uy_l = field_c[0] < h_small ? 0.0 : field_c[3] / field_c[0];

  //   ux_r = field_cell_r[0] < h_small ? 0.0 : field_cell_r[2] /
  //   field_cell_r[0];
  //   uy_r = field_cell_r[0] < h_small ? 0.0 : field_cell_r[3] /
  //   field_cell_r[0];
  // }
...
// template <typename scalar_t>
// __device__ __forceinline__ void
// hllcFluxSolver_wetDry(scalar_t *field_c, scalar_t *field_l, scalar_t
// *field_r,
//                       const scalar_t *normal, const scalar_t *tangential,
//                       scalar_t &flux_h, scalar_t &flux_qx, scalar_t &flux_qy,
//                       scalar_t &flux_h_dry, scalar_t &flux_qx_dry,
//                       scalar_t &flux_qy_dry, const scalar_t h_small,
...
//   scalar_t h_star = pow(a_L / 2.0 + u_L / 4.0, 2.0) / g;
//   scalar_t u_star = u_L / 2.0 + a_L;
//   scalar_t a_star = sqrt(g * h_star);
...
//   scalar_t h_flux_R = 0.0;
//   scalar_t qx_flux_R = 0.0;
//   scalar_t qy_flux_R = 0.0;
...
//   flux_h -= (h_flux / dx) * dt;
//   flux_qx -= ((qx_flux * normal[0] + qy_flux * tangential[0] +
//                0.5 * g * (field_l[0] + field_c[0]) * (field_l[4] -
//                field_c[4]) *
//                    normal[0]) /
//               dx) *
//              dt;
...
//   flux_h_dry += (h_flux / dx) * dt;
//   flux_qx_dry += ((qx_flux * normal[0] + qy_flux * tangential[0]) / dx) * dt;
//   flux_qy_dry += ((qx_flux * normal[1] + qy_flux * tangential[1]) / dx) * dt;
// }
  ...
  if (w < M) {
    ...
    for (int j = 0; j < 4; j++) {
      ...
      if (r_id == -1) {
        ...
        // singleSlope_MUSCL_reconstruction(field_c, field_l, field_r,
        // field_e_l,
        //                                  h_small);
      } else {
        ...
        // z_inter = max(field_e_l[4], field_e_r[4]);
        // if (field_e_l[1] - z_inter < h_small ||
        //     field_e_r[1] - z_inter < h_small || field_c[0] < h_small ||
        //     field_r[0] < h_small) {
        //   for (int v = 0; v < 5; v++) {
        //     field_e_l[v] = field_c[v];
        //     field_e_r[v] = field_r[v];
        //   }
        // }
        // jingming's limitation
        // if (field_e_l[0] <=
        //         min(abs(field_e_l[4] - field_c[4]), 0.25 * field_c[0]) ||
        //     field_e_r[0] <=
        //         min(abs(field_e_r[4] - field_r[4]), 0.25 * field_r[0]) ||
        //     field_c[0] < h_small || field_r[0] < h_small) {
        //   for (int v = 0; v < 5; v++) {
        //     field_e_l[v] = field_c[v];
        //     field_e_r[v] = field_r[v];
        //   }
