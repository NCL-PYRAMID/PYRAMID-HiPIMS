...
hydroStaticReconstruction(scalar_t *field_c_h_z, scalar_t *field_r_h_z,
  ...
  // ==================================================
  // from hipims xilin AWR
  // ==================================================
  // scalar_t delta_b_interface = field_r[4] - field_l[4];

  // scalar_t delta_b = field_r_h_z[1] - field_c_h_z[1];

  // field_l[1] += max(0.0, min(delta_b - delta_b_interface,
  //                            (field_r_h_z[0] + field_r_h_z[1]) -
  //                                (field_c_h_z[0] + field_c_h_z[1])));
  // field_r[1] += max(0.0, min(delta_b_interface - delta_b,
  //                            (field_c_h_z[0] + field_c_h_z[1]) -
  //                                (field_r_h_z[0] + field_r_h_z[1])));

  // scalar_t z_bm = max(field_c_h_z[1], field_r_h_z[1]);

  // field_l[0] = max(field_l[1] - z_bm, 0.0);
  // field_r[0] = max(field_r[1] - z_bm, 0.0);

  // if (field_r_h_z[0] > h_small) {
  //   field_l[4] = z_bm - max(0.0, min(delta_b_interface,
  //                                    z_bm - (field_c_h_z[0] +
  //                                    field_c_h_z[1])));
  // } else {
  //   field_l[4] = z_bm - max(0.0, z_bm - field_c_h_z[0] - field_c_h_z[1]);
  // }

  // field_r[4] = field_r[1] - field_r[0];

  // Xilin, Jiaheng translate
  ...
  if (field_r_h_z[0] < h_small) {
    field_l[4] = min(field_l[4], field_c_h_z[0] + field_c_h_z[1]);
  } else {
    // xilin
    field_l[4] = z_bm - max(0.0, min(delta_b_interface,
                                     z_bm - (field_c_h_z[0] + field_c_h_z[1])));
    //  jiaheng
    // field_l[4] = field_l[1] - field_l[0];
    // jiaheng & xilin
    // field_l[4] = z_bm - max(0.0, min(delta_b_interface, z_bm - field_l[1]));
  }

  // jiaheng
  // scalar_t z_bm = min(max(field_l[4], field_r[4]), min(field_l[1],
  // field_r[1]));
  // field_l[0] = min(max(field_l[1] - z_bm, 0.0), field_l[0]);
  // field_r[0] = min(max(field_r[1] - z_bm, 0.0), field_r[0]);
  // field_l[4] = field_l[1] - field_l[0];
  // field_r[4] = field_r[1] - field_r[0];

  // =========================================================
  // hydrostatic reconstruction from Guoxian Chen and Sebastian Noelle
  //  A new hydrostatic reconstruction scheme motivated by the wet-dry front
  // =========================================================

  // auto z_bm = min(max(field_l[1] - field_l[0], field_r[1] - field_r[0]),
  //                 min(field_l[1], field_r[1]));
  ...
  // field_l[0] = max(field_c_h_z[0] + field_c_h_z[1] - z_bm, 0.0);
  // field_r[0] = max(field_r_h_z[0] + field_r_h_z[1] - z_bm, 0.0);
}
  ...
  if (w < M) {
    ...
    for (int j = 0; j < 4; j++) {
      ...
      if (r_id == -1) {
        ...
        // minmod_tri(field_r[4] - field_rr_z, field_c[4] - field_rr_z,
        //            field_c[4] - field_r[4], r_b);
      ...
      if (field_c[0] < h_small && field_r[0] < h_small) {
        ...
        // if (c_id == 5000) {
        //   for (int p = 0; p < 5; p++) {
        //     printf("Before: left values [%d]: %f \n", p, field_c[p]);
        //     printf("Before: Reft values [%d]: %f \n", p, field_r[p]);
        //   }
        //   printf("Left z: %f \n", field_l_z);
        //   printf("Rightright z: %f \n", field_rr_z);
        //   for (int p = 0; p < 2; p++) {
        //     printf("Before: left averaged values [%d]: %f \n", p,
        //            field_c_h_z[p]);
        //     printf("Before: Reft averaged values [%d]: %f \n", p,
        //            field_r_h_z[p]);
        //   }
        // }
        ...
        // if (c_id == 5000) {
        //   for (int p = 0; p < 5; p++) {
        //     printf("After: left values [%d]: %f \n", p, field_c[p]);
        //     printf("After: Reft values [%d]: %f \n", p, field_r[p]);
        //   }

        //   for (int p = 0; p < 2; p++) {
        //     printf("After: left averaged values [%d]: %f \n", p,
        //            field_c_h_z[p]);
        //     printf("After: Reft averaged values [%d]: %f \n", p,
        //            field_r_h_z[p]);
        //   }
        // }
        ...
        // HR method
        // qx_flux[i] -=
        //     ((0.5 * g * (-field_c[0] * field_c[0]) * _normal[0]) / dx[0]) *
        //     dt[0];
        // qy_flux[i] -=
        //     ((0.5 * g * (-field_c[0] * field_c[0]) * _normal[1]) / dx[0]) *
        //     dt[0];
        // qx_flux[i] -= ((0.5 * g * (field_c_h_z[0] * field_c_h_z[0] -
        //                            field_c[0] * field_c[0]) *
        //                 _normal[0]) /
        //                dx[0]) *
        //               dt[0];
        // qy_flux[i] -= ((0.5 * g * (field_c_h_z[0] * field_c_h_z[0] -
        //                            field_c[0] * field_c[0]) *
        //                 _normal[1]) /
        //                dx[0]) *
        //               dt[0];
