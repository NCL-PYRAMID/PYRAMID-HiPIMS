...
singleSlope_MUSCL_reconstruction(scalar_t *field_c, scalar_t *field_l,
  ...
  field_edge[2] =
      field_c[0] < h_small ? 0.0 : field_edge[0] * field_c[2] / field_c[0];

  field_edge[3] =
      field_c[0] < h_small ? 0.0 : field_edge[0] * field_c[3] / field_c[0];

  // field_edge[0] = field_c[0];
  // field_edge[1] = field_c[1];
  // field_edge[2] = field_c[2];
  // field_edge[3] = field_c[3];
  // field_edge[4] = field_c[4];
}
...
__global__ void fluxCalculation_kernel(
  ...
  if (w < M) {
    ...
    for (int j = 0; j < 4; j++) {
      ...
      _normal_minus[0] = -_normal[0];
      _normal_minus[1] = -_normal[1];
