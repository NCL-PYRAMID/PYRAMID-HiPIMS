...
template <typename scalar_t>
// 由于give_eta参数不方便处理，因此此处采用given_depth代替水位
__device__ __forceinline__ void 
boundary_eta_given(scalar_t *field, scalar_t *field_b, const scalar_t given_depth) {
  if (given_depth>field[4]){
    field_b[0] = given_depth - field[4];
    field_b[1] = given_depth;
  }
  else {
    field_b[0] = 0;
    field_b[1] = field_b[4];
...
boundary_conditions(const int boundaryType, scalar_t *field, scalar_t *field_b,
  ...
  case 8: {
    boundary_eta_given(field, field_b, given_depth);
    break;
  }
  case 9: {
