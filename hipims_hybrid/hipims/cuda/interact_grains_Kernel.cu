...
__global__ void interact_grains_kernel(int np, scalar_t *__restrict__ mu, 
	scalar_t *__restrict__ g_gkn_device, scalar_t *__restrict__ g_gnu_device, scalar_t *__restrict__ g_grho_device, scalar_t *__restrict__ g_gkt_device, 
	...
	scalar_t *__restrict__ neloc_x_device, scalar_t *__restrict__ neloc_y_device){
	...
	if (a<np) {
		for (b = a + 1; b < np; b++){
			...
			if (sqrt(dx*dx + dy*dy) <= dL){
				interparticle_force(a, b, mu[0],g_gkn_device, g_gnu_device, g_grho_device,  g_gkt_device, np, dt_g[0], g_gne_device, g_gbar_device, g_glay_device, g_gR_device,
...
template <typename scalar_t>
__device__ __forceinline__ void interparticle_force(int a, int b, scalar_t mu, scalar_t *g_gkn_device, scalar_t *g_gnu_device, scalar_t *g_grho_device, scalar_t *g_gkt_device, 
			...
			for (int r = 0; r < g_gne_device[b]; r++){
				for (int j = 0; j < g_gbar_device[b]; j++){
					...
					if (dn < 0.0) {   //Contact
						...
						fn = -g_gkn_device[a] * dn - g_gnu_device[a] * vn;
						ft = fabs(g_gkt_device[a] * vt*dt_g);
...
void interact_grains_cuda(at::Tensor mu, at::Tensor kn, at::Tensor gnu, at::Tensor grho, at::Tensor gkt, 
    ...
    AT_DISPATCH_FLOATING_TYPES(
        gx.type(), "interact_grains_cuda", ([&] {
          interact_grains_kernel<scalar_t><<<block_0, thread_0, 0, stream>>>(
              np,mu.data<scalar_t>(), kn.data<scalar_t>(), gnu.data<scalar_t>(), grho.data<scalar_t>(), gkt.data<scalar_t>(), 
							dt_g.data<scalar_t>(), ne.data<int32_t>(), bar.data<int32_t>(), lay.data<int32_t>(), gR.data<scalar_t>(),
							gx.data<scalar_t>(), gy.data<scalar_t>(), gvx.data<scalar_t>(), gvy.data<scalar_t>(),
							gax.data<scalar_t>(), gay.data<scalar_t>(), gang.data<scalar_t>(), gangv.data<scalar_t>(), ganga.data<scalar_t>(),
							gfx.data<scalar_t>(), gfy.data<scalar_t>(), gfz.data<scalar_t>(), gp.data<scalar_t>(), gt.data<scalar_t>(),
							ex.data<scalar_t>(), ey.data<scalar_t>());
        }));
