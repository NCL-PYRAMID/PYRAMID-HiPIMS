...
__global__ void parInit_kernel(const int NN, const int N, const int PNN, 
                                scalar_t *x, scalar_t *y, scalar_t *dx,
                                int32_t *Ms_cum, int32_t *Mg_cum, 
                                int32_t *cellid, scalar_t *xp, scalar_t *yp,
                                int32_t *layer, int32_t *pclass){
    ...
    if (j < NN){
        ...
        int Tol_cumN = Ms_cum[NN-1]+Mg_cum[NN-1];

        if (j==0){
            for (int i=0; i<Ms0; i++){
                cellid[i] = k;
                xp[i] = x[k] - 0.5 * dx[0] + (0.5 + i) * dx[0] / Ms0;
                yp[i] = y[k] - 0.5 * dx[0] + (0.5 + i) * dx[0] / Ms0;
                // xp[i] = x[j];
                // yp[i] = y[j];
            ...
            for (int i=Ms0; i<Tol_cum0; i++){
                cellid[i] = k;
                xp[i] = x[k] - 0.5 * dx[0] + (0.5 + i - Ms0) * dx[0] / (Tol_cum0 - Ms0);
                yp[i] = y[k] - 0.5 * dx[0] + (0.5 + i - Ms0) * dx[0] / (Tol_cum0 - Ms0);
                // xp[i] = x[j];
                // yp[i] = y[j];
            ...
            for (int i=Tol_cumj1; i<Tol_cumj2; i++){
                cellid[i] = k;
                xp[i] = x[k] - 0.5 * dx[0] + (0.5 + i - Tol_cumj1) * dx[0] / (Tol_cumj2 - Tol_cumj1);
                yp[i] = y[k] - 0.5 * dx[0] + (0.5 + i - Tol_cumj1) * dx[0] / (Tol_cumj2 - Tol_cumj1);
                // xp[i] = x[j];
                // yp[i] = y[j];
                layer[i] = 1;
                pclass[i] = c;
            ...
            for (int i=Tol_cumj2; i<Tol_cum; i++){
                cellid[i] = k;
                xp[i] = x[k] - 0.5 * dx[0] + (0.5 + i - Tol_cumj2) * dx[0] / (Tol_cum - Tol_cumj2);
                yp[i] = y[k] - 0.5 * dx[0] + (0.5 + i - Tol_cumj2) * dx[0] / (Tol_cum - Tol_cumj2);
                // xp[i] = x[j];
                // yp[i] = y[j];
                layer[i] = 2;
                pclass[i] = c;
...
void parInit_cuda(at::Tensor x, at::Tensor y, at::Tensor dx,
                    ...
                    at::Tensor layer, at::Tensor pclass) {
    at::cuda::CUDAGuard device_guard(x.device());
    cudaStream_t stream = at::cuda::getCurrentCUDAStream();

    const int NN = Ms_cum.numel();
    const int N = x.numel();
    ...
    AT_DISPATCH_FLOATING_TYPES(
        x.type(), "polInit_cuda", ([&] {
          parInit_kernel<scalar_t><<<block_0, thread_0, 0, stream>>>(
              NN, N, PNN, x.data<scalar_t>(), y.data<scalar_t>(), dx.data<scalar_t>(),
