...
def ProcessingBar(t,tout,start,scale=100):
    i=int(t*100/tout)
    a = '*' * i
    b = '.' * (scale - i)
    c = (i/scale)*100
    ...
    if gauge_index_1D.size()[0] > 0:
        ...
        while numerical.t.item() < paraDict['EndTime']:
            ...
            if dt_tol >= dt_washoff:
                ...
                dt_tol += numerical.dt
            ProcessingBar(t, paraDict['EndTime'], simulation_start)
            n += 1
    else:
        while numerical.t.item() < paraDict['EndTime']:
            numerical.addFlux()
            ...
            if dt_tol >= dt_washoff:
                ...
                dt_tol += numerical.dt
            ProcessingBar(t, paraDict['EndTime'], simulation_start)
    pollutant.get_c(numerical.get_h(), dx)
    simulation_end = time.perf_counter()
    print('Running time: %s Seconds'%(simulation_end-simulation_start))

    np.savetxt(paraDict['OUTPUT_PATH'] + '/Ms.txt', np.array(pollutant._Ms))
    np.savetxt(paraDict['OUTPUT_PATH'] + '/Mg.txt', np.array(pollutant._Mg))
    np.savetxt(paraDict['OUTPUT_PATH'] + '/c.txt', np.array(pollutant._c))
    np.savetxt(paraDict['OUTPUT_PATH'] + '/h.txt', np.array(numerical.get_h()))
