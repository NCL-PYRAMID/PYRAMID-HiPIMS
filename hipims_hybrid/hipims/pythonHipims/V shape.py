...
def run(paraDict):
    ...
    if paraDict['secondOrder']:
        while numerical.t.item() < paraDict['EndTime']:
            qx0 = numerical.get_qx()
            qy0 = numerical.get_qy()
            u0 = qx0/numerical.get_h()
            v0 = qy0/numerical.get_h()
            numerical.rungeKutta_update(rainfallMatrix, device)
            numerical.time_update_cuda(device)
            pollutant.transport(numerical.get_h(), numerical.get_qx(), numerical.get_qy(), u0, v0, numerical.dt)
